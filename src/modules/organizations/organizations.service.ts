...
export class OrganizationsService {
  constructor(
    ...
    private readonly teams: Repository<OrganizationTeam>,
    @InjectRepository(TeamMember)
    private readonly teamMembers: Repository<TeamMember>,
    @InjectRepository(TeamConversation)
    private readonly teamConvs: Repository<TeamConversation>,
    ...
    @InjectRepository(OrgTask)
    private readonly tasks: Repository<OrgTask>,
    @InjectRepository(OrgMeeting)
    private readonly meetings: Repository<OrgMeeting>,
    @InjectRepository(OrgMeetingAttendee)
    private readonly meetingAttendees: Repository<OrgMeetingAttendee>,
    @InjectRepository(OrgAttendanceLog)
    private readonly attendanceLogs: Repository<OrgAttendanceLog>,
    @InjectRepository(OrgApproval)
    private readonly approvals: Repository<OrgApproval>,
    @InjectRepository(OrgPraise)
    private readonly praises: Repository<OrgPraise>,
    @InjectRepository(OrgNotification)
    private readonly notifications: Repository<OrgNotification>,
    @InjectRepository(OrgShift)
    private readonly shifts: Repository<OrgShift>,
    @InjectRepository(OrgActivityLog)
    private readonly activities: Repository<OrgActivityLog>,
    @InjectRepository(OrgCallLog)
    private readonly orgCallLogs: Repository<OrgCallLog>,
    @InjectRepository(OrgCallLogParticipant)
    private readonly orgCallParticipants: Repository<OrgCallLogParticipant>,
    @InjectRepository(User)
    private readonly users: Repository<User>,
    @InjectRepository(OrgCalendarEvent)
    private readonly calendarEvents: Repository<OrgCalendarEvent>,
    @InjectRepository(OrgCalendarEventAttendee)
    private readonly calendarAttendees: Repository<OrgCalendarEventAttendee>,
    private readonly gateway: RealtimeGateway,
  ) {}

  private isManagerRole(role: OrganizationMember["role"]) {
  ...
  private async getOrgMembership(userId: string, organizationId: string) {
    ...
    if (!membership || membership.status !== "active") {
      throw new ForbiddenException("Not an active organization member");
    }
    return membership;
  }
  ...
  private async assertOrgManager(userId: string, organizationId: string) {
    ...
    if (!this.isManagerRole(membership.role)) {
      throw new ForbiddenException(
        "Only owner/admin/manager can do this action",
      );
    }
    return membership;
  }
  ...
  private async canManageTeam(
    ...
    if (this.isManagerRole(orgMember.role)) return true;
  ...
  private async assertCanViewTeam(
    ...
    if (team.visibility === "private") {
      ...
      if (!isManager) {
        ...
        if (!teamMember)
          throw new ForbiddenException("Private team access denied");
      }
    }
    return team;
  }

  private async getMainConversationId(teamId: string) {
  ...
  private async getTeamUserIds(teamId: string) {
    const rows = await this.teamMembers.find({ where: { team_id: teamId } });
    return rows.map((row) => row.user_id);
  }

  private async createMeetingCallLog(
    organizationId: string,
    teamId: string,
    meetingId: string,
    initiatedBy: string,
    ...
    const callLog = await this.orgCallLogs.save(
      this.orgCallLogs.create({
        organization_id: organizationId,
        team_id: teamId,
        // call_id references calls.id, so we keep it null for org meetings.
        call_id: null,
    ...
    if (teamUserIds.length) {
      await this.orgCallParticipants.save(
        teamUserIds.map((uid) =>
          this.orgCallParticipants.create({
  ...
  private async getActiveMeetingCallLog(meetingId: string) {
  ...
  private toClock(date?: Date | null) {
    if (!date) return "-";
  ...
  private toShortDate(date?: Date | null) {
    if (!date) return null;
  ...
  private async mapTeamSummary(team: OrganizationTeam) {
  ...
  async create(userId: string, dto: CreateOrganizationDto) {
    ...
    const organization = await this.orgs.save(
      this.orgs.create({
        name: dto.name.trim(),
    ...
    await this.orgMembers.save(
      this.orgMembers.create({
    ...
    if (inviteIds.length) {
      ...
      if (users.length !== uniqueIds.length) {
        throw new BadRequestException("Some users not found for member invite");
      }
      await this.orgMembers.save(
        uniqueIds.map((uid) =>
          this.orgMembers.create({
  ...
  async update(userId: string, id: string, dto: UpdateOrganizationDto) {
    ...
    if (dto.slug !== undefined) patch.slug = dto.slug?.trim() || null;
  ...
  async delete(userId: string, id: string) {
    ...
    if (membership.role !== "owner" && organization.created_by !== userId) {
      throw new ForbiddenException(
        "Only owner/creator can delete organization",
      );
    }
  ...
  async addMembers(
    ...
    const existing = await this.orgMembers.find({
      where: { organization_id: organizationId, user_id: In(ids) },
    });
  ...
  async updateMemberRole(
    ...
    if (myMembership.role !== "owner" && role === "owner") {
      throw new ForbiddenException("Only owner can promote another owner");
    }
    await this.orgMembers.update(
    ...
    if (teamIds.length) {
      ...
      if (convLinks.length) {
        await this.participants.delete({
          conversation_id: In(convLinks.map((c) => c.conversation_id)),
          user_id: memberId,
        });
      }
    }
  ...
  async leave(userId: string, organizationId: string) {
    ...
    const teamIds = teams.map((t) => t.id);
    if (teamIds.length) {
      await this.teamMembers.delete({ team_id: In(teamIds), user_id: userId });
      const links = await this.teamConvs.find({
        where: { team_id: In(teamIds) },
      });
      if (links.length) {
        await this.participants.delete({
          conversation_id: In(links.map((l) => l.conversation_id)),
          user_id: userId,
        });
      }
    }
  ...
  async listTeams(userId: string, organizationId: string) {
    ...
    // Filter teams to only include those where user is a member
  ...
  async getTeam(userId: string, organizationId: string, teamId: string) {
  ...
  async createTeam(userId: string, organizationId: string, dto: CreateTeamDto) {
    await this.assertOrgManager(userId, organizationId);
    if (!dto.name?.trim()) throw new BadRequestException("Team name required");
    ...
    if (!leadMembership)
      throw new BadRequestException("Lead user must be an active org member");
    ...
    await this.teamConvs.save(
      this.teamConvs.create({
        team_id: team.id,
        conversation_id: conversation.id,
        type: "main",
      }),
    ...
    toAdd.forEach((uid) =>
      this.gateway.addUserToConversationRoom(uid, conversation.id),
    );
  ...
  async updateTeam(
    ...
    if (!canManage)
      throw new ForbiddenException(
        "Only org managers or team lead can update team",
      );

    const patch: Partial<OrganizationTeam> = {};
    if (dto.name !== undefined) patch.name = dto.name.trim();
    if (dto.description !== undefined) patch.description = dto.description;
    ...
    if (dto.lead_user_id !== undefined) {
      ...
      if (!leadMembership)
        throw new BadRequestException("Lead must be active org member");
  ...
  async deleteTeam(userId: string, organizationId: string, teamId: string) {
    ...
    if (!canManage)
      throw new ForbiddenException(
        "Only org managers or team lead can delete team",
      );
  ...
  async addTeamMembers(
    ...
    if (!canManage)
      throw new ForbiddenException(
        "Only org managers or team lead can add members",
      );
    ...
    if (!filtered.length) return { ok: true, added: 0 };

    const existing = await this.teamMembers.find({
      where: { team_id: teamId, user_id: In(filtered) },
    });
    const existingSet = new Set(existing.map((m) => m.user_id));
    ...
    if (conversationId) {
      ...
      toAdd.forEach((uid) =>
        this.gateway.addUserToConversationRoom(uid, conversationId),
      );
    }
  ...
  async setTeamMemberRole(
    ...
    if (!canManage)
      throw new ForbiddenException(
        "Only org managers or team lead can change roles",
      );
  ...
  async removeTeamMember(
    ...
    if (!canManage)
      throw new ForbiddenException(
        "Only org managers or team lead can remove members",
      );
    ...
    if (team.lead_user_id === memberId) {
      ...
      await this.teams.update(
        { id: teamId },
        { lead_user_id: nextLead?.user_id || null },
      );
      if (nextLead) {
  ...
  async createTask(
    ...
    if (dto.assignee_id && dto.assignee_id !== userId) {
      ...
      await this.createNotification(
        ...
        "task",
        "Task Assigned",
        `${creatorName} assigned you to "${dto.title.trim()}"`,
        {
          task_id: task.id,
          creator: creatorName,
          creator_id: userId,
  ...
  async deleteTask(
    ...
    if (!canManage && task.created_by !== userId)
      throw new ForbiddenException(
        "Only the task creator or team admin can delete this task",
      );
  ...
  async updateTask(
    ...
    await this.assertCanViewTeam(userId, organizationId, teamId);
    if (dto.status)
      await this.tasks.update({ id: taskId }, { status: dto.status as any });
  ...
  async scheduleMeeting(
    ...
    if (!canManage) {
      throw new ForbiddenException(
        "Only org managers or team lead can schedule meetings",
      );
    }
    ...
    const attendeeIds = Array.from(new Set(dto.attendee_ids || [])).filter(
      (id) => allowedIds.has(id),
    );
    const finalAttendees = attendeeIds.length ? attendeeIds : teamUserIds;

    if (finalAttendees.length) {
      await this.meetingAttendees.save(
        finalAttendees.map((uid) =>
          this.meetingAttendees.create({
    ...
    for (const attendeeId of finalAttendees) {
      if (attendeeId !== userId) { // Don't notify the organizer
        await this.createNotification(
          ...
          "meeting",
          "Meeting Scheduled",
          `${organizerName} scheduled "${dto.title.trim()}" for ${startsAt.toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}`,
          {
            meeting_id: meeting.id,
  ...
  async startMeetingNow(
    ...
    if (!canManage) {
      throw new ForbiddenException(
        "Only org managers or team lead can start meetings",
      );
    }
    ...
    const callLog = await this.createMeetingCallLog(
      organizationId,
      teamId,
      meeting.id,
      userId,
  ...
  async startMeeting(
    ...
    const callLog =
      existing ||
      (await this.createMeetingCallLog(
        organizationId,
        teamId,
        meetingId,
        userId,
    ...
    // Create notifications for all team members about meeting start
    ...
    for (const uid of teamUserIds) {
      if (uid !== userId) { // Don't notify the person who started it
        await this.createNotification(
          organizationId,
          uid,
          "meeting",
          "Meeting Started",
          `${starterName} started "${meeting.title}"`,
          {
            meeting_id: meetingId,
            room_id: callLog.id,
            started_by: userId,
  ...
  async endMeeting(
    ...
    if (!canManage) {
      throw new ForbiddenException(
        "Only org managers or team lead can end meetings",
      );
    }
    ...
    if (activeLog) {
      ...
      const duration = Math.max(
        0,
        Math.floor(
          (endedAt.getTime() - new Date(activeLog.started_at).getTime()) / 1000,
        ),
      );
      await this.orgCallLogs.update(
        { id: activeLog.id },
        {
          status: "ended",
          ended_at: endedAt,
    ...
    // Create notification for all team members about meeting ended
    ...
    for (const uid of teamUserIds) {
      if (uid !== userId) {
        await this.createNotification(
          ...
          "meeting",
          "Meeting Ended",
          `${endedByName} ended the meeting "${meeting.title}"`,
          {
            meeting_id: meetingId,
            team_id: teamId,
            ended_by: userId,
  ...
  async createActivity(
    ...
    organizationId: string,
    teamId: string,
    dto: { activity_type: string; preview_text?: string },
  ) {
    // file_shared can be logged by any team member; other types require manager/lead
    const isFileShared = dto.activity_type === "file_shared";
    const isAutoLog = ["file_shared", "task_created", "task_updated"].includes(
      dto.activity_type,
    );
    if (!isAutoLog) {
      const canManage = await this.canManageTeam(
        userId,
        organizationId,
        teamId,
      );
      if (!canManage)
        throw new ForbiddenException(
          "Only org managers or team lead can add activities",
        );
    } else {
      await this.getOrgMembership(userId, organizationId);
    }
    ...
    const activity = await this.activities.save(
      this.activities.create({
        ...
        team_id: teamId,
        actor_id: userId,
        activity_type: dto.activity_type as any,
  ...
  async deleteActivity(
    ...
    if (!canManage)
      throw new ForbiddenException(
        "Only org managers or team lead can delete activities",
      );
  ...
  async getTeamWorkspace(
    ...
    const teamUsers = teamUserIds.length
      ? await this.users.find({ where: { id: In(teamUserIds) } })
      : [];
    ...
    if (conversationId) {
      ...
      for (const reaction of reactions) {
        ...
        if (!list.includes(reaction.emoji)) list.push(reaction.emoji);
    ...
    const taskAssigneeIds = Array.from(
      new Set(
        taskRows.map((t) => t.assignee_id).filter((id): id is string => !!id),
      ),
    );
    const taskAssignees = taskAssigneeIds.length
      ? await this.users.find({ where: { id: In(taskAssigneeIds) } })
      : [];
    ...
    // Fetch calendar events (now at organization level, team_id is nullable)
    let calendar: Array<{
      id: string;
      title: string;
      description?: string;
    ...
    try {
      // Fetch org-level calendar events (team_id is NULL for org-wide events)
      const calendarEventRows = await this.calendarEvents.find({
        where: { organization_id: organizationId, team_id: IsNull() },
      ...
      for (const attendee of eventAttendees) {
        ...
        if (user) list.push(user.display_name);
      ...
      // If calendar tables don't exist yet, just return empty array
    ...
    const latestByUser = new Map<string, OrgAttendanceLog>();
    ...
    const approvals = approvalRows.map((approval) => ({
      ...
      requester_name:
        userMap.get(approval.requested_by)?.display_name || "Unknown",
    ...
    // Remove team-level praise (now organization-level only)
    const praise: any[] = [];
    ...
    const shifts = shiftRows.map((shift) => ({
      ...
      role:
        teamMemberRows.find((tm) => tm.user_id === shift.user_id)?.role ===
        "lead"
          ? "Lead"
          : "Member",
  ...
  async createCalendarEvent(
    ...
    const event = this.calendarEvents.create({
      organization_id: organizationId,
      team_id: null, // Organization-level event
      title: payload.title,
    ...
    if (payload.attendee_ids && payload.attendee_ids.length > 0) {
      const attendees = payload.attendee_ids.map((attendeeId) =>
        this.calendarAttendees.create({
          event_id: event.id,
          user_id: attendeeId,
    ...
    // Emit to all organization members
    if (this.gateway) {
    ...
    // Create notifications for all organization members about new calendar event
    const creator = await this.users.findOne({ where: { id: userId } });
    const creatorName = creator?.display_name || "Someone";
    const orgMemberRows = await this.orgMembers.find({
      where: { organization_id: organizationId },
    ...
    for (const uid of orgUserIds) {
      if (uid !== userId) { // Don't notify the creator
        await this.createNotification(
          organizationId,
          uid,
          "activity",
          "New Calendar Event",
          `${creatorName} added "${payload.title}" on ${payload.date}`,
          {
            event_id: event.id,
            creator: creatorName,
            creator_id: userId,
  ...
  async deleteCalendarEvent(
    ...
    const event = await this.calendarEvents.findOne({
      where: {
        id: eventId,
        organization_id: organizationId,
        team_id: IsNull(), // Only org-level events
      },
    });
  ...
  async clockIn(userId: string, organizationId: string, teamId: string) {
    await this.assertCanViewTeam(userId, organizationId, teamId);
    ...
    if (existing) {
      ...
      await this.attendanceLogs.update(
        { id: existing.id },
        {
          sign_in_at: new Date(),
          status: "active",
  ...
  async clockOut(userId: string, organizationId: string, teamId: string) {
    await this.assertCanViewTeam(userId, organizationId, teamId);
    ...
    if (!log.sign_in_at) {
      throw new BadRequestException("Invalid clock-in record");
    }

    const signOutTime = new Date();
    const workMinutes = Math.floor(
      (signOutTime.getTime() - new Date(log.sign_in_at).getTime()) /
        (1000 * 60),
    );
  ...
  async orgClockIn(userId: string, organizationId: string) {
    await this.getOrgMembership(userId, organizationId);
  ...
  async orgClockOut(userId: string, organizationId: string) {
    await this.getOrgMembership(userId, organizationId);
  ...
  async getOrgAttendance(userId: string, organizationId: string) {
    await this.getOrgMembership(userId, organizationId);
  ...
  async createApproval(
    ...
    const approval = this.approvals.create({
      ...
      team_id: teamId,
      requested_by: userId,
      approval_type: payload.approval_type as any,
      title: payload.title,
      description: payload.description || null,
    ...
    // Emit to team members (especially admins)
    if (this.gateway) {
      const teamMemberRows = await this.teamMembers.find({
        where: { team_id: teamId },
      });
    ...
    for (const tm of teamMemberRows) {
      if (tm.role === "lead" && tm.user_id !== userId) {
        await this.createNotification(
          ...
          "approval",
          "Approval Request",
          `${requesterName} requested ${payload.approval_type}: "${payload.title}"`,
          {
            approval_id: approval.id,
  ...
  async approveApproval(
    ...
    await this.createNotification(
      ...
      "approval",
      "Request Approved",
      `${approverName} approved your ${approval.approval_type} request: "${approval.title}"`,
      {
        approval_id: approvalId,
  ...
  async rejectApproval(
    ...
    await this.createNotification(
      ...
      "approval",
      "Request Rejected",
      `${approverName} rejected your ${approval.approval_type} request: "${approval.title}"`,
      {
        approval_id: approvalId,
        approver: approverName,
        approver_id: userId,
  ...
  // Remove team-level praise method (now organization-level only)
  ...
  async sendOrgPraise(
    ...
    // Validate recipient is an organization member
    const recipientMember = await this.orgMembers.findOne({
      where: { organization_id: organizationId, user_id: payload.to_user_id },
    });

    if (!recipientMember) {
      throw new BadRequestException("Recipient is not an organization member");
    }

    if (payload.to_user_id === userId) {
      throw new BadRequestException("You cannot praise yourself");
    }

    const praise = this.praises.create({
      organization_id: organizationId,
      team_id: null, // Organization-level praise
      from_user_id: userId,
    ...
    await this.createNotification(
      ...
      "praise",
      "New Praise Received",
      `${fromUser?.display_name || "Someone"} praised you with ${payload.badge} badge`,
      {
    ...
    // Emit to recipient and all organization members
    if (this.gateway) {
      const orgMemberRows = await this.orgMembers.find({
        where: { organization_id: organizationId },
      });
  ...
  async getOrgPraise(userId: string, organizationId: string) {
    await this.getOrgMembership(userId, organizationId);

    // Get all organization-level praise (where team_id is NULL)
  ...
  async getOrgCalendar(userId: string, organizationId: string) {
    await this.getOrgMembership(userId, organizationId);

    // Get organization-level calendar events (where team_id is NULL)
    const calendarEventRows = await this.calendarEvents.find({
      where: { organization_id: organizationId, team_id: IsNull() },
      order: { date: "ASC", start_time: "ASC" },
      take: 200,
    ...
    // Get ALL meetings in the organization where the user is an attendee
    // This includes meetings from all teams in the organization
  ...
  async markNotificationAsRead(
    ...
    // Emit to user for real-time update
    if (this.gateway) {
  ...
  async markAllNotificationsAsRead(userId: string, organizationId: string) {
    ...
    await this.notifications.update(
      { organization_id: organizationId, user_id: userId, read: false },
      { read: true, read_at: new Date() },
    );
