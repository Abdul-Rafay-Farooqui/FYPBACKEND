...
let BatchesService = class BatchesService {
    ...
    async findAll(institute_id, search, sortField = batches_dto_1.SortField.YEAR, sortOrder = batches_dto_1.SortOrder.DESC, page = 1, limit = 10) {
        ...
        if (sortField === batches_dto_1.SortField.STUDENT_COUNT) {
    ...
    async getBatchWithStudents(batchId) {
    ...
    async addStudentsToBatch(batchId, studentIds, classBatchSectionId) {
        await this.findOne(batchId);
        const cbs = await this.cbsRepo.findOne({
            where: { id: classBatchSectionId, batch_id: batchId }
        });
        if (!cbs) {
            throw new common_1.BadRequestException('Invalid class_batch_section for this batch');
        }
    ...
    async removeStudentFromBatch(enrollmentId) {
    ...
    async updateStudentEnrollment(enrollmentId, classBatchSectionId) {
        const enrollment = await this.enrollmentRepo.findOne({
            where: { id: enrollmentId }
        });
        if (!enrollment) {
    ...
    async delete(id) {
        ...
        if (cbsIds.length > 0) {
            ...
            if (enrollmentCount > 0) {
                throw new common_1.BadRequestException('Cannot delete batch with active student enrollments');
            }
        }
