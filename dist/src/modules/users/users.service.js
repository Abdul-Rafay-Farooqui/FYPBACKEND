...
let UsersService = class UsersService {
    ...
    async setPresence(id, is_online) {
        await this.users.update({ id }, { is_online, last_seen: new Date() });
    }
    async search(q, excludeId) {
        if (!q || !q.trim())
            return [];
        const searchTerm = `%${q}%`;
        return this.users.find({
            where: [
                { phone: (0, typeorm_2.Like)(searchTerm) },
                { display_name: (0, typeorm_2.Like)(searchTerm) },
                { username: (0, typeorm_2.Like)(searchTerm) },
